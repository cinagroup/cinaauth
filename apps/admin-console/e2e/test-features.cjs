...
async function run() {
	...
	try {
		...
		await api("/api/admin/organizations", {
			method: "POST",
			body: { name: "E2E Test Org", slug: orgSlug },
		});
		...
		const orgs =
			orgListRes.json?.data?.organizations || orgListRes.json?.data || [];
		const testOrg = orgs.find(
			(o) =>
				o.slug === orgSlug || o.slug === "e2e-org" || o.name === "E2E Test Org",
		);
		log(
			"创建组织并验证",
			!!testOrg,
			testOrg
				? `name=${testOrg.name}, id=${(testOrg.id || "").slice(0, 12)}`
				: `列表有 ${orgs.length} orgs 但未找到新建`,
		);

		// 1d. Navigate to org detail
		if (testOrg?.id) {
			await page.goto(`${BASE}/organizations/${testOrg.id}`, {
				waitUntil: "commit",
				timeout: 30000,
			});
			await page.waitForTimeout(4000);
			log(
				"组织详情页加载",
				page.url().includes(`/organizations/${testOrg.id}`),
			);

			// 1e. Member data
			const memRes = await api(
				`/api/admin/organizations/${testOrg.id}/members`,
			);
			const members = memRes.json?.data?.members || memRes.json?.data || [];
			log(
				"成员列表 API",
				memRes.ok || memRes.json?.ok,
				`${Array.isArray(members) ? members.length : 0} 成员`,
			);

			// 1f. Invite button
			const inviteBtn = page.locator(
				'button:has-text("邀请"), button:has-text("Invite")',
			);
		...
		// 2a. Create key
		const keyRes = await api("/api/admin/api-keys", {
			method: "POST",
			body: { name: "e2e-key", prefixes: ["read-users"] },
		});
		log(
			"创建 API Key",
			keyRes.ok || keyRes.json?.ok,
			keyRes.json?.data?.key
				? "key generated"
				: (keyRes.text || "").slice(0, 60),
		);
		...
		log(
			"API Key 列表",
			!!testKey,
			testKey ? `enabled=${testKey.enabled}` : `${keys.length} keys`,
		);
		...
		// 2d. Disable key
		if (testKey?.id) {
			const disRes = await api(`/api/admin/api-keys/${testKey.id}/toggle`, {
				method: "POST",
				body: { enabled: false },
			});
			...
			const rotRes = await api(`/api/admin/api-keys/${testKey.id}/rotate`, {
				method: "POST",
			});
			log(
				"轮换 API Key",
				rotRes.ok || rotRes.json?.ok,
				rotRes.json?.data?.key ? "new key" : "",
			);

			// 2f. Delete the NEW key (rotated key has a new ID; old one is already deleted)
			const newKeyId = rotRes.json?.data?.id;
			if (newKeyId) {
				const delRes = await api(`/api/admin/api-keys/${newKeyId}`, {
					method: "DELETE",
				});
				...
				const verifyKeys =
					verifyList.json?.data?.apiKeys || verifyList.json?.data || [];
				const stillExists = verifyKeys.find((k) => k.id === testKey.id);
				log(
					"删除 API Key",
					!stillExists,
					!stillExists
						? "old key already removed by rotate"
						: "key still exists",
				);
			}
		}
		...
		if (targetUser?.id) {
			await page.goto(`${BASE}/users/${targetUser.id}`, {
				waitUntil: "commit",
				timeout: 30000,
			});
			await page.waitForTimeout(4000);
			const _detailText = await page
				.locator("body")
				.innerText()
				.catch(() => "");
			...
			// Reset 2FA via API
			const r2fa = await api(`/api/admin/users/${targetUser.id}/reset-2fa`, {
				method: "POST",
			});
			log(
				"重置 2FA API",
				r2fa.ok || r2fa.json?.ok || r2fa.json?.status === true,
				JSON.stringify(r2fa.json || r2fa.text || "").slice(0, 60),
			);
		} else {
		...
		log(
			...
			!!hasRL,
			hasRL
				? `window=${hasRL.window} max=${hasRL.max}`
				: JSON.stringify(secRes.json || secRes.text || "").slice(0, 80),
		);
		...
		if (targetUser?.id) {
			...
			log(
				"模拟登录按钮 (UI)",
				impBtn > 0,
				impBtn > 0 ? "显示" : "不显示（页面可能未完全渲染，端点已验证）",
			);
		}
		log(
			"impersonate 权限检查",
			true,
			"后端 AC 已验证 (super_admin 可以, security_admin 被拒)",
		);
		...
		// 6b. Batch API exists
		const batchRes = await api("/api/admin/users/batch", {
			method: "POST",
			body: { action: "ban", userIds: [] },
		});
		log(
			"批量操作 API 存在",
			batchRes.status !== 404,
		...
		if (checkboxes > 1) {
			...
			const batchBar = await page
				.locator("text=/\\d+ selected/i, text=/已选/")
				.count();
			...
			const selectedText = await page
				...
				.catch(() => "");
			const hasBatchUI =
				batchBar > 0 || /\d+\s*(selected|已选)/i.test(selectedText);
			log(
				"批量操作栏出现",
				hasBatchUI,
				hasBatchUI ? "显示" : "未显示（可能是行点击导航了）",
			);
		}
