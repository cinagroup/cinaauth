...
describe("request-state", () => {
	...
	describe("AsyncLocalStorage init race", () => {
		it("shares one AsyncLocalStorage across concurrent first-callers", async () => {
			...
			vi.resetModules();
			const cinaAuthGlobalSymbol = Symbol.for("cinaauth:global");
			const cinaAuthGlobal = (globalThis as any)[cinaAuthGlobalSymbol];
			if (cinaAuthGlobal?.context) {
				cinaAuthGlobal.context.requestStateAsyncStorage = undefined;
			}
