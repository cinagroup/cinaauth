 ...
 * making `jwtClient()` unassignable to `CinaAuthClientPlugin` and collapsing
...
describe("jwtClient types", () => {
	it("should be assignable to CinaAuthClientPlugin", () => {
		const plugin: CinaAuthClientPlugin = jwtClient();
		const plugins: CinaAuthClientPlugin[] = [jwtClient()];
		expect(plugin.id).toBe("cinaauth-client");
