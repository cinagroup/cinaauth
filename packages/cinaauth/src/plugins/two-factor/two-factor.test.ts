		...
		const verifyRes = await client.twoFactor.verifyOtp({
			...
			fetchOptions: {
				...
				onResponse(context) {
					...
					expect(parsed.get("cinaauth.session_token")?.value).not.toBeDefined();
				},
			},
		});
		...
		await client.twoFactor.sendOtp({
			fetchOptions: {
				...
				onSuccess(context) {
					...
					headers.append(
						"cookie",
						`cinaauth.otp.counter=${parsed.get("cinaauth.otp_counter")?.value}`,
					);
				},
			},
		});
...
describe("trust device server-side validation", async () => {
	...
	it("should force 2FA when server-side trust record is expired", async () => {
		...
		const trustDeviceCookieValue = parsed.get("cinaauth.trust_device")?.value;
	...
	it("should preserve trust device after sign-out", async () => {
		...
		const trustCookieAfterSignOut = signOutParsed.get("cinaauth.trust_device");
