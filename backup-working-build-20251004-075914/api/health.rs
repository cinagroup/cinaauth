...
/// GET /metrics
/// Prometheus metrics endpoint (returns plain text for Prometheus compatibility)
...
auth_framework_requests_total{{method="POST",endpoint="/api/v1/auth/login"}} 892
auth_framework_requests_total{{method="GET",endpoint="/api/v1/users/profile"}} 654
...
pub async fn liveness_check(State(_state): State<ApiState>) -> Json<ApiResponse<LivenessResponse>> {
    ...
    let response = LivenessResponse {
        alive,
        message: if alive {
            "Service is alive".to_string()
        } else {
            "Service is dead".to_string()
        },
    };
