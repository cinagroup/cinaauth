...
Cinaauth provides enterprise-grade key management integration via PKCS#11, allowing cryptographic operations to be offloaded to robust external appliances (HSMs) or cloud KMS solutions.
...
3. The `hsm` feature flag enabled when compiling Cinaauth.
...
1. Stop the Cinaauth instances to ensure no key rotations occur.
...
Cinaauth caches public keys in memory to ensure that signature verification remains ultra-fast, primarily relying on the HSM for the private key signing operations.
