...
# CinaAuth 1.2 – Stripe, Captcha, API Keys, Teams, Init CLI, and more

To upgrade, run:

```package-install
npm install cinaauth@1.2
```
...
```ts title="auth.ts"
import { stripe } from "@cinaauth/stripe"; // [!code highlight]
...
```ts title="auth.ts"
import { captcha } from "cinaauth/plugins";
...
The CLI now includes an `init` command to add CinaAuth to your project.
...
We rewrote **better-call** (the core library behind CinaAuth) to fix TypeScript editor lag. Your IDE should now feel much snappier when working with CinaAuth.
...
A lot of fixes and refinements to make everything smoother, faster, and more reliable. Check out the [changelog](https://github.com/cinagroup/cinaauth/releases/tag/v1.2.0) for more details.

***

```package-install
npm install cinaauth@1.2.0
```
