---
title: Installation
description: Learn how to configure CinaAuth in your project.
---
...
<Steps>
  <Step>
    ...
    Let's start by adding CinaAuth to your project:
    ...
    <Callout type="info">
      If you're using a separate client and server setup, make sure to install CinaAuth in both parts of your project.
    </Callout>
  </Step>
  ...
  <Step>
    ...
    <Callout type="info">
      Need to rotate your secret later? You can use `CINAAUTH_SECRETS` (plural) to roll over to a new secret without invalidating existing data. See the [`secrets` option](/docs/reference/options#secrets) for details.
    </Callout>

    2. **Set Base URL**

    ```txt title=".env"
    CINAAUTH_URL=http://localhost:3000 # Base URL of your app
    ```
  </Step>
  ...
  <Step>
    ...
    * `utils/` folder
    ...
    And in this file, import CinaAuth and create your auth instance. Make sure to export the auth instance with the variable name `auth` or as a `default` export.
  ...
  <Step>
    ...
    CinaAuth requires a database to store user data.
    You can easily configure CinaAuth to use SQLite, PostgreSQL, or MySQL, and more!

    <Callout>
      You can also configure CinaAuth to work in a stateless mode if you don't configure a database. See [Stateless Session Management](/docs/concepts/session-management#stateless-session-management) for more information. Note that most plugins will require a database.
    </Callout>
    ...
    <Tabs items={["drizzle", "prisma", "mongodb"]}>
      ...
      <Tab value="mongodb">
        ```ts title="auth.ts"
        import { CinaAuth } from "cinaauth";
        import { mongodbAdapter } from "cinaauth/adapters/mongodb";
    ...
    <Callout>
      ...
      consider importing `CinaAuth` from `cinaauth/minimal` instead of `cinaauth` to reduce bundle size.
  ...
  <Step>
    ...
    CinaAuth includes a CLI tool to help manage the schema required by the library.
  ...
  <Step>
    ...
    Configure the authentication methods you want to use. CinaAuth comes with built-in support for email/password, and social sign-on providers.
  ...
  <Step>
    ...
    <Callout>
      CinaAuth supports any backend framework with standard Request and Response
    ...
    <Tabs items={["next-js-app-router", "next-js-pages-router", "nuxt", "svelte-kit", "react-router", "solid-start", "hono", "cloudflare-workers", "express", "elysia", "tanstack-start", "expo"]} defaultValue="next-js-app-router">
      ...
      <Tab value="cloudflare-workers">
        ...
        <Callout type="info">
          **Node.js AsyncLocalStorage Support**: CinaAuth uses AsyncLocalStorage for async context tracking. To enable this in Cloudflare Workers, add the `nodejs_compat` flag to your `wrangler.toml`:
      ...
      <Tab value="express">
        ...
        app.listen(port, () => {
            console.log(`CinaAuth app listening on port ${port}`);
        });
        ```
      ...
      <Tab value="elysia">
        ...
        const CinaAuthView = (context: Context) => {
            const CINAAUTH_ACCEPT_METHODS = ["POST", "GET"]
            // validate request method
            if(CINAAUTH_ACCEPT_METHODS.includes(context.request.method)) {
        ...
        const app = new Elysia().all("/api/auth/*", CinaAuthView).listen(3000);
      ...
      <Tab value="tanstack-start">
        ...
        <Callout type="info">
          When you call functions that need to set cookies (like `signInEmail` or `signUpEmail`), you'll need to handle cookie setting for TanStack Start. CinaAuth provides a `tanstackStartCookies` plugin to automatically handle this for you.
  ...
  <Step>
    ...
    The client-side library helps you interact with the auth server. CinaAuth comes with a client for all the popular web frameworks, including vanilla JavaScript.

    1. Import `createAuthClient` from the package for your framework (e.g., "cinaauth/react" for React).
...
"vanilla"]}
    ...
    >
      ...
      <Tab value="svelte" title="lib/auth-client.ts">
        ```ts title="lib/auth-client.ts"
        import { createAuthClient } from "cinaauth/svelte"
        export const authClient = createAuthClient({
            /** The base URL of the server (optional if you're using the same domain) */ // [!code highlight]
            baseURL: "http://localhost:3000" // [!code highlight]
        })
  ...
  <Step>
    ...
    That's it! You're now ready to use cinaauth in your application. Continue to [basic usage](/docs/basic-usage) to learn how to use the auth instance to sign in users.
  </Step>
</Steps>
