...
`code` parameter (and typically `state`). Without the `code`, CinaAuth cannot exchange it for tokens,
...
* Start the flow through CinaAuth so the provider receives the correct parameters and the app expects a `code`.
