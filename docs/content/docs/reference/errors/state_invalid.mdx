...
When using the **cookie** state storage strategy (`account.storeStateStrategy: "cookie"`), CinaAuth encrypts
...
* **Secret rotation mid-flow.** The `CINAAUTH_SECRET` was changed between the start and the callback of the OAuth
...
Use your browser's DevTools (Application, then Cookies) to confirm that the `cinaauth.oauth_state` cookie is set
