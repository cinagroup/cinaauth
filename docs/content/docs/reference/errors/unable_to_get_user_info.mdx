...
the authorization code for tokens, CinaAuth fetches the user's profile from the provider. If the
...
* Start the OAuth flow using CinaAuth methods so the correct scopes and parameters are used.
