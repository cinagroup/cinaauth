...
During the OAuth callback, CinaAuth expects a `state` value to be present on the incoming request.
...
### Start the flow via CinaAuth APIs

Always initiate OAuth through CinaAuth so we can generate and send `state` correctly. Avoid manually hitting
callback endpoints or constructing authorize URLs unless you fully mirror CinaAuth's parameters.
