...
This error occurs when CinaAuth fails to create a session after a successful authentication step. A session is required to keep the user logged in, so failure to create one results in this error.
...
* Verify your CinaAuth configuration for session handling.
