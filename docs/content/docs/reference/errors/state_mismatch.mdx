...
When an OAuth or SSO flow begins, CinaAuth generates a unique `state` value and stores it so it can be
...
CinaAuth supports two state storage strategies - **database** (the default) and **cookie** - and each
...
5. **Secret rotation with hashed identifiers.** If `verification.storeIdentifier` is `"hashed"`, the
   identifier is hashed using the server secret. Changing `CINAAUTH_SECRET` between the start and
...
* Do not rotate `CINAAUTH_SECRET` during active OAuth flows, or run both old and new secrets during
  a transition window.
...
* `CINAAUTH_SECRET` was rotated between the start and callback of the flow, so the decryption key
  no longer matches.
...
After the verification record is found in the database, CinaAuth also checks that a **signed state
...
* **OAuth Proxy with mismatched secrets:** When using the [OAuth Proxy plugin](/docs/plugins/oauth-proxy) and production/preview have different `CINAAUTH_SECRET` values without a shared `secret` configured in the plugin options, the proxy's before hook fails to decrypt the state package, causing the regular callback handler to run and fail.

### How to fix

* Use a **stable, custom domain** and verify cookie attributes.
* For SAML flows, CinaAuth already sets `skipStateCookieCheck` internally.
...
2. **Open DevTools → Application → Cookies.** Confirm the state cookie (`cinaauth.state` or
   `cinaauth.oauth_state`) is set before the redirect and still present when the callback arrives.
