---
title: FAQ
description: Frequently asked questions about CinaAuth.
---

This page contains frequently asked questions, common issues, and other helpful information about CinaAuth.
...
<Accordions>
  ...
  <Accordion id="corporate-proxy-configuration" title="How to configure a corporate proxy for server-side requests">
    If you need to configure a corporate proxy for CinaAuth's outbound requests (e.g., OAuth provider requests), you can use undici's ProxyAgent to set up a global fetch proxy.
    ...
    Since CinaAuth runs on the server side, you have full control over Node.js's fetch behavior through undici's global dispatcher. This approach allows all outbound requests from CinaAuth to go through your corporate proxy.
  ...
  <Accordion id="adding-custom-fields-to-the-users-table" title="Adding custom fields to the users table">
    CinaAuth provides a type-safe way to extend the user and session schemas, take a look at our docs on <Link href="/docs/concepts/database#extending-core-schema">extending core schema</Link>.
  </Accordion>
  ...
  <Accordion id="dual-module-hazard-issue" title="Dual Module Hazard Issue">
    ...
    This is typically caused by a **dual module hazard** - when multiple versions of `cinaauth` or `@cinaauth/core` exist in your dependency tree. This commonly occurs after upgrading to v1.4+, especially in Cloudflare Workers, Nuxt, or when using plugins like `oauthProvider`.

    ### How to Diagnose

    Check if you have duplicate versions of CinaAuth packages:

    ```bash
    # For pnpm users
    pnpm why @cinaauth/core
    pnpm why cinaauth

    # For npm users
    npm ls @cinaauth/core
    npm ls cinaauth

    # For yarn users
    yarn why @cinaauth/core
    yarn why cinaauth
    ```
    ...
    Start by ensuring your dependencies are clean and all CinaAuth packages use compatible versions.
    ...
    <Callout type="tip">
      **Clean reinstall:**
      ...
      Check your `package.json` and make sure all CinaAuth related packages (`cinaauth`, `@cinaauth/core`, `@cinaauth/oauth-provider`, etc.) are using compatible versions.
    </Callout>
    ...
    <Callout type="error">
      **Important:** Make sure to install `cinaauth` and related packages in `dependencies`, not `devDependencies`.
    </Callout>
    ...
    <Callout type="info">
      **For Next.js users:**
      ...
      Add `cinaauth` to `serverExternalPackages` in your `next.config.js`:

      ```ts title="next.config.js"
      const config = {
        serverExternalPackages: ['cinaauth']
      };
      ```
    </Callout>
    ...
    <Callout type="info">
      Related issues: [#6613](https://github.com/cinagroup/cinaauth/issues/6613), [yjs#438](https://github.com/yjs/yjs/issues/438)
    </Callout>
  </Accordion>
</Accordions>
