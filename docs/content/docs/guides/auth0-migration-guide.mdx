...
<Steps>
  ...
  <Step>
    ...
    // helper function to migrate password from auth0 to CinaAuth for custom hashes and algs
                ...
                try {
                    ...
                    for (const member of members.data || []) {
                        try {
                            ...
                            const role = mapAuth0RoleToCinaAuthRole(userRoles.data?.map(r => r.name) || []);
                            await ctx.adapter.create({
                                model: "member",
                                data: {
