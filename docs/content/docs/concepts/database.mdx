...
CinaAuth provides an official Redis storage package that uses [ioredis](https://github.com/redis/ioredis):

```bash
npm install @cinaauth/redis-storage ioredis
# or
pnpm add @cinaauth/redis-storage ioredis
```
...
This implementation allows CinaAuth to use Redis for storing session data and rate limiting counters. You can also add prefixes to the keys names.
...
import { SecondaryStorage } from "cinaauth";
...
Finally, we can pass the implementation to the `CinaAuth` function.
...
Since CinaAuth version `1.4` we've introduced experimental database joins support.
...
The CinaAuth `1.4` CLI will generate DrizzleORM and PrismaORM relationships for you so if you do not have those already
