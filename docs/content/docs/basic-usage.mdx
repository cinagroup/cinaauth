---
title: Basic Usage
description: Getting started with CinaAuth
---

CinaAuth provides built-in authentication support for:
...
```ts title="server.ts"
import { auth } from "./auth"; // path to your CinaAuth server instance
...
CinaAuth supports multiple social providers, including Google, GitHub, Apple, Discord, and more. To use a social provider, you need to configure the ones you need in the `socialProviders` option on your `auth` object.
...
Once a user is signed in, you'll want to access the user session. CinaAuth allows you to easily access the session data from both the server and client sides.

### Client Side

#### Use Session

CinaAuth provides a `useSession` hook to easily access session data on the client side. This hook is implemented using nanostore and has support for each supported framework and vanilla client, ensuring that any changes to the session (such as signing out) are immediately reflected in your UI.
...
<Tabs items={["Next.js", "Nuxt", "Svelte", "Astro", "Hono", "TanStack"]}>
  ...
  <Tab value="react-router">
    ```ts title="route.ts"
    import { auth } from "lib/auth"; // path to your CinaAuth server instance
...
One of the unique features of CinaAuth is a plugins ecosystem. It allows you to add complex auth related functionality with small lines of code.
...
<Steps>
  <Step>
    ...
    ```ts title="auth.ts"
    import { CinaAuth } from "cinaauth"
    import { twoFactor } from "cinaauth/plugins" // [!code highlight]
  ...
  <Step>
    ...
    import { twoFactorClient } from "cinaauth/client/plugins"; // [!code highlight]
