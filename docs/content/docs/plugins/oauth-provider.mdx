...
Verification can be performed using `verifyAccessToken` available through the `oauthProviderResourceClient` plugin or `cinaauth/oauth2` package.
...
<Steps>
  ...
  <Step>
    ...
    * Using `mcpHandler` helper
    ...
    `mcpHandler` accepts the same `jwksFetch` and `jwksCacheKey` alternatives
    as `verifyAccessToken` when the resource server must use a private
    transport such as a Cloudflare Service Binding.
  </Step>
</Steps>
...
```ts
import { createHash } from "@cinaauth/utils/hash";
import { base64Url } from "@cinaauth/utils/base64";
