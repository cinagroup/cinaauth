...
<Steps>
  <Step>
    ...
    import { lastLoginMethod } from "cinaauth/plugins" // [!code highlight]
  ...
  <Step>
    ...
    import { lastLoginMethodClient } from "cinaauth/client/plugins" // [!code highlight]
...
<Steps>
  <Step>
    ...
    ```ts title="auth.ts"
    import { CinaAuth } from "cinaauth"
    import { lastLoginMethod } from "cinaauth/plugins"
...
export const auth = CinaAuth({
    plugins: [
        lastLoginMethod({
            ...
            cookieName: "cinaauth.last_used_login_method", // Default: "cinaauth.last_used_login_method"
...
```ts title="auth-client.ts"
import { createAuthClient } from "cinaauth/client"
import { lastLoginMethodClient } from "cinaauth/client/plugins"
...
The plugin automatically inherits cookie settings from CinaAuth's centralized cookie system. This solves the problem where the last login method wouldn't persist across:

* **Cross-subdomain setups**: `auth.example.com` → `app.example.com`
* **Cross-origin setups**: `api.company.com` → `app.different.com`

When you enable `crossSubDomainCookies` or `crossOriginCookies` in your CinaAuth config, the plugin will automatically use the same domain, secure, and sameSite settings as your session cookies, ensuring consistent behavior across your application.
...
When using CinaAuth with Expo, make sure to import the client plugin from `@cinaauth/expo/plugins` rather than from `cinaauth/plugins/client`. This ensures the last login method is stored correctly using the configured storage.

```ts
import { createAuthClient } from "cinaauth/react"
import { expoClient } from "@cinaauth/expo"
import { lastLoginMethodClient } from "@cinaauth/expo/plugins" // [!code highlight]
