---
title: MCP
description: MCP provider plugin for CinaAuth
---
...
<Steps>
  <Step>
    ...
    import { mcp } from "cinaauth/plugins"; // [!code highlight]
...
import { oAuthProtectedResourceMetadata } from "cinaauth/plugins";
import { auth } from "@/lib/auth";
...
import { withMcpAuth } from "cinaauth/plugins";
import { z } from "zod";
...
The examples above use `withMcpAuth` which requires the CinaAuth instance to be in the **same process** as the MCP server. If your MCP server runs as a separate service (different repo, different runtime, different language), you can use the **MCP Client** — a lightweight, framework-agnostic HTTP client that validates Bearer tokens against a remote CinaAuth server.

<Callout>
  No additional packages needed — the MCP Client is included in `cinaauth`.
</Callout>
...
<Steps>
  <Step>
    ...
    Point it at your CinaAuth server's URL (the same `baseURL` + `basePath` from your auth config):

    ```ts title="mcp-server.ts"
    import { createMcpAuthClient } from "cinaauth/plugins/mcp/client" // [!code highlight]
  ...
  <Step>
    ...
    // GET /.well-known/oauth-authorization-server → proxied from CinaAuth
    // GET /.well-known/oauth-protected-resource → points MCP clients to CinaAuth
    ```

    These proxy and cache the metadata from your CinaAuth server, so MCP clients can discover the authorization, token, and registration endpoints automatically.
  </Step>
</Steps>
...
<Tabs items={["Hono", "Express", "Official MCP SDK", "mcp-use"]}>
  <Tab value="Hono">
    ...
    import { mcpAuthHono } from "cinaauth/plugins/mcp/client/adapters" // [!code highlight]
  ...
  <Tab value="Official MCP SDK">
    ...
    import { mcpAuthOfficial } from "cinaauth/plugins/mcp/client/adapters" // [!code highlight]
  ...
  <Tab value="mcp-use">
    ...
    import { mcpAuthMcpUse } from "cinaauth/plugins/mcp/client/adapters" // [!code highlight]
...
export const mcpClientOptionsType = {
  authURL: {
    description: "Full URL to CinaAuth endpoints (baseURL + basePath)",
    type: "string",
    required: true,
  },
  resource: {
