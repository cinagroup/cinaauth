---
title: Polar
description: CinaAuth Plugin for Payment and Checkouts using Polar
---

[Polar](https://polar.sh) is a developer first payment infrastructure. Out of the box it provides a lot of developer first integrations for payments, checkouts and more. This plugin helps you integrate Polar with CinaAuth to make your auth + payments flow seamless.
...
```bash
pnpm add cinaauth @polar-sh/cinaauth @polar-sh/sdk
```
...
import { polar, checkout, portal, usage, webhooks } from "@polar-sh/cinaauth";
...
### Configuring CinaAuth Client

You will be using the CinaAuth Client to interact with the Polar functionalities.

```typescript
import { createAuthClient } from "cinaauth/react";
import { polarClient } from "@polar-sh/cinaauth/client";

// This is all that is needed
// All Polar plugins, etc. should be attached to the server-side CinaAuth config
...
When `createCustomerOnSignUp` is enabled, a new Polar Customer is automatically created when a new User is added in the CinaAuth Database.
...
```typescript
import { polar, checkout } from "@polar-sh/cinaauth";
...
When checkouts are enabled, you're able to initialize Checkout Sessions using the checkout-method on the CinaAuth Client. This will redirect the user to the Product Checkout.
...
```typescript
import { polar, checkout, portal } from "@polar-sh/cinaauth";
...
The portal-plugin gives the CinaAuth Client a set of customer management methods, scoped under `authClient.customer`.
...
```typescript
import { polar, checkout, portal, usage } from "@polar-sh/cinaauth";
...
```typescript
import { polar, webhooks } from "@polar-sh/cinaauth";
