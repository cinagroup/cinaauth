...
* **Flexible Architecture** - Use CinaAuth endpoints or direct server-side functions
...
Configure CinaAuth with the Creem plugin:

```typescript
// lib/auth.ts
import { CinaAuth } from "cinaauth";
import { creem } from "@creem_io/cinaauth";
...
import { creemClient } from "@creem_io/cinaauth/client";
...
```typescript
// lib/auth-client.ts
import { createCreemAuthClient } from "@creem_io/cinaauth/create-creem-auth-client";
import { creemClient } from "@creem_io/cinaauth/client";
...
<Callout type="info">
  Depending on your database adapter, additional setup steps may be required. Refer to the [CinaAuth adapter documentation](https://www.cinagroup.com/docs/adapters/mysql) for details.
</Callout>
...
<Steps>
  <Step>
    ...
    (`/api/auth` is the default CinaAuth server path)
...
import { validateWebhookSignature } from "@creem_io/cinaauth/server";
...
Use these utilities directly in Server Components, Server Actions, or API routes without going through CinaAuth endpoints.
...
import {
  ...
  getDaysUntilRenewal,
  validateWebhookSignature,
} from "@creem_io/cinaauth/server";
```

### Server Component Example

```typescript
import { checkSubscriptionAccess } from "@creem_io/cinaauth/server";
...
import { createCheckout } from "@creem_io/cinaauth/server";
import { auth } from "@/lib/auth";
import { headers } from "next/headers";
import { redirect } from "next/navigation";
...
* [CinaAuth Documentation](https://cinagroup.com)
* [Plugin GitHub Repository Additional Documentation](https://github.com/armitage-labs/creem-CinaAuth)

## Support

For issues or questions:

* Open an issue on [GitHub](https://github.com/armitage-labs/creem-CinaAuth/issues)
