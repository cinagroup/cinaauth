...
<Steps>
  ...
  <Step>
    ...
    import { passkey } from "@cinaauth/passkey" // [!code highlight]
  ...
  <Step>
    ...
    import { passkeyClient } from "@cinaauth/passkey/client" // [!code highlight]
...
```ts title="passkey-list.tsx"
import { commonAuthenticatorNames } from "@cinaauth/passkey";
...
```ts title="auth.ts"
import { CinaAuth } from "cinaauth";
import { passkey, getAuthenticatorName } from "@cinaauth/passkey";
...
Deleting a passkey requires a fresh, authoritative session according to your configured [`session.freshAge`](/docs/concepts/session-management#session-freshness). A stale session receives a `403 SESSION_NOT_FRESH` response. Client applications should send the user through an explicit sign-in or step-up flow before retrying; they must not treat that response as a successful deletion.
...
**origin**: The origin URL at which your cinaauth server is hosted. `http://localhost` and `http://localhost:PORT` are also valid. Do NOT include any trailing /.
...
* `webAuthnChallengeCookie`: Cookie name for storing WebAuthn challenge ID during authentication flow (Default: `cinaauth-passkey`)
...
By default, the passkey plugin uses `"cinaauth-passkey"` as the challenge cookie name. Since this starts with `"cinaauth"`, it will work with the default Expo client configuration. However, if you customize the `webAuthnChallengeCookie` option, you must also update the `cookiePrefix` in your Expo client configuration.
