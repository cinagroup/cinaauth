...
## Stats & Wallet Endpoints (CinaGroup Console)

The admin plugin additionally exposes read/aggregation endpoints for the
`admin.cinagroup.com` dashboard, plus SIWE wallet management. All require the
`admin` (or, where noted, `security_admin`) role via the `stats:read` and
`wallet:list`/`wallet:unbind` permission statements.
...
`security_admin`: A scoped admin role for the CinaGroup console. Can list/get
users, ban/unban, manage sessions (revoke), manage SIWE wallets (list/unbind),
and read stats — but cannot create or delete users, set roles, impersonate,
set passwords, or edit security policy. Intended for security operators.
