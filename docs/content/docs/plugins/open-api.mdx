---
title: Open API
description: Open API reference for CinaAuth.
---

This is a plugin that provides an Open API reference for CinaAuth. It shows all endpoints added by plugins and the core. It also provides a way to test the endpoints. It uses [Scalar](https://scalar.com/) to display the Open API reference.
...
<Steps>
  <Step>
    ...
    import { openAPI } from "cinaauth/plugins" // [!code highlight]

    export const auth = CinaAuth({
        plugins: [
            openAPI({
              title: "Acme Identity",
              description: "Identity and access management API for Acme",
            }), // [!code highlight]
        ]
    })
    ```
...
If you're using Scalar for your API documentation, you can add CinaAuth as an additional source alongside your main API:

When using Hono with Scalar for OpenAPI documentation, you can integrate CinaAuth by adding it as a source:
...
app.get("/docs", Scalar({
  ...
  sources: [
    ...
    // CinaAuth schema generation endpoint
...
This allows you to display both your application's API and CinaAuth's authentication endpoints in a unified documentation interface.

`title` - Sets one public API title across the generated OpenAPI schema, the HTML document title, and Scalar metadata. When omitted, the plugin preserves its established defaults: `CinaAuth` in the schema, `Scalar API Reference` in the HTML document, and `CinaAuth API` in Scalar metadata.

`description` - The public API description used by both the generated OpenAPI schema and Scalar metadata. Default is `API Reference for your CinaAuth Instance`.
