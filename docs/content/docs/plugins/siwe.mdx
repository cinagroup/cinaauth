...
```ts
// 1. Request a nonce and sign a standard ERC-4361 message in the wallet.
...
`linkWallet`, `setPrimaryWallet`, and `unlinkWallet` require a fresh,
authoritative session. The caller cannot supply a `userId`; the current session
always determines ownership. Linking consumes the nonce once and verifies the
same domain, address, Chain ID, time bounds, and signature as wallet login.
