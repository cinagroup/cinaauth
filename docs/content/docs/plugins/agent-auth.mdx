...
<Callout type="warn">
  This plugin is an implementation of a standard on heavy development. It's not yet stable and may change in the future. Please report any issues or bugs on [Github](https://github.com/cinagroup/cinaauth/agent-auth).
</Callout>

The Agent Auth plugin lets your CinaAuth server act as an **Agent Auth provider**. It's a server implementation of the [Agent Auth Protocol](https://agentauthprotocol.com).
...
<Steps>
  ...
  <Step>
    ...
    import { agentAuth } from "@cinaauth/agent-auth"; // [!code highlight]
  ...
  <Step>
    ### Optional: add the CinaAuth client plugin

    If you want type-safe access to the plugin endpoints from a CinaAuth client, add the client plugin too.

    ```ts title="auth-client.ts"
    import { createAuthClient } from "cinaauth/client";
    import { agentAuthClient } from "@cinaauth/agent-auth/client"; // [!code highlight]
...
* **`issuer`** — The provider’s base URL (CinaAuth `baseURL`).
...
<Callout>
  The discovery route should live at `/.well-known/agent-configuration`, even if your CinaAuth base path is `/api/auth`.
</Callout>
...
import { createFromOpenAPI } from "@cinaauth/agent-auth/openapi";
...
} from "@cinaauth/agent-auth/openapi";
...
import { verifyAgentRequest } from "@cinaauth/agent-auth";
...
Types are exported from **`@cinaauth/agent-auth`** (for example **`AgentSession`**).
