...
<Steps>
  ...
  <Step>
    ...
    <Callout>
      CinaAuth also supports Facebook Login for Business, all you need
      to do is provide the `configId` as listed in **Facebook Login For Business > Configurations** alongside your `clientId` and `clientSecret`. Note that the app must be a Business app and, since CinaAuth expects to have an email address and account id, the configuration must be of the "User access token" type. "System-user access token" is not supported.
    </Callout>
  ...
  <Step>
    ...
    * `provider`: The provider to use. It should be set to `facebook`.
    ...
    import { createAuthClient } from "cinaauth/auth-client"
...
For limited login, you need to pass `idToken.token`, for only `accessToken` you need to pass `idToken.accessToken` and `idToken.token` together because of [#1183](https://github.com/cinagroup/cinaauth/issues/1183).
