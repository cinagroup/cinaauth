...
To handle already signed-in users, you can use the `electron.transferUser` method on the client and `electronTransferUser` on the server to transfer the user to the Electron app. [See Example](https://github.com/cinagroup/cinaauth/blob/main/demo/nextjs/app/\(auth\)/sign-in/_components/electron.tsx)
