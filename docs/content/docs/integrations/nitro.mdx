...
description: Integrate CinaAuth with Nitro.
---

CinaAuth can be integrated with your [Nitro Application](https://nitro.build/) (an open source framework to build web servers).

This guide aims to help you integrate CinaAuth with your Nitro application in a few simple steps.
...
### Install & Configure CinaAuth

Follow steps 1 & 2 from the [installation guide](/docs/installation) to install CinaAuth in your Nitro application & set up the environment variables.

Once that is done, create your CinaAuth instance within the `server/utils/auth.ts` file.
...
Use the CinaAuth CLI to update your Prisma schema with the required models by running the following command:
...
<Callout>
  The `--config` flag is used to specify the path to the file where you have created your CinaAuth instance.
</Callout>
...
You can now mount the CinaAuth handler in your Nitro application. You can do this by adding the following code to your `server/routes/api/auth/[...all].ts` file:
...
See an [example Nitro application integrated with CinaAuth & Prisma](https://github.com/BayBreezy/nitrojs-cinaauth-prisma).
