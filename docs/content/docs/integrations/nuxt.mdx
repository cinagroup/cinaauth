...
description: Integrate CinaAuth with Nuxt.
---
...
Mount the CinaAuth handler to a catch-all Nitro route. Create a file at `server/api/auth/[...all].ts`:
...
<Callout type="info">
  You can change the mount path in your CinaAuth configuration, but keeping it as `/api/auth/[...all]` is recommended.
</Callout>

## Create a client

Create an auth client using `cinaauth/vue` so `useSession` returns Vue refs:
...
* [`atinux/nuxthub-cinaauth`](https://github.com/atinux/nuxthub-cinaauth)
* [`leamsigc/nuxt-cinaauth-drizzle`](https://github.com/leamsigc/nuxt-cinaauth-drizzle)
