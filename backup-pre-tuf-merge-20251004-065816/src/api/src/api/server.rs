...
use crate::api::{ApiState, admin, auth, health, mfa, middleware, oauth, oauth2, users};
use axum::{
    Router,
...
impl ApiServer {
    ...
    pub async fn build_router(&self) -> crate::errors::Result<Router> {
        ...
        let router = Router::new()
            ...
            .route("/oauth/token-exchange", post(oauth::token_exchange))
            .route(
                "/.well-known/openid-configuration",
                get(oauth::oidc_discovery),
            )
            .route("/.well-known/jwks.json", get(oauth::jwks))
            .route("/oidc/userinfo", get(oauth::userinfo))
            // TUF-Laptop OAuth2 Integration (Versioned API)
            .route("/api/v1/oauth2/authorize", get(oauth2::authorize))
            .route("/api/v1/oauth2/token", post(oauth2::token))
            .route("/api/v1/oauth2/revoke", post(oauth2::revoke))
            .route("/api/v1/oauth2/userinfo", get(oauth2::userinfo))
            ...
            .route("/api-keys/revoke", post(auth::revoke_api_key))
