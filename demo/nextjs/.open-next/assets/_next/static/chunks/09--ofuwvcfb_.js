...
`,
		...
		const eL = {
			...
			eP = Symbol.for("cinaauth:broadcast-channel");
		var eM = class {
			listeners = new Set();
			name;
		...
		function ek(e = "cinaauth.message") {
			return globalThis[eP] || (globalThis[eP] = new eM(e)), globalThis[eP];
		}
		const eY = Symbol.for("cinaauth:focus-manager");
		var eB = class {
			listeners = new Set();
			subscribe(e) {
				return (
		...
		function eF() {
			return globalThis[eY] || (globalThis[eY] = new eB()), globalThis[eY];
		}
		const eH = Symbol.for("cinaauth:online-manager");
		var eV = class {
			listeners = new Set();
		...
		function e$() {
			return globalThis[eH] || (globalThis[eH] = new eV()), globalThis[eH];
		}
		const ez = () => Math.floor(Date.now() / 1e3);
		function ej(e) {
		...
		var eX = Object.defineProperty,
			...
			e2 = (e, t, r) =>
				t in e
					...
					: (e[t] = r),
			e4 = (e, t) => {
				for (var r in t || (t = {})) e0.call(t, r) && e2(e, r, t[r]);
				if (eQ) for (var r of eQ(t)) e1.call(t, r) && e2(e, r, t[r]);
				return e;
			},
			e5 = (e, t) => eZ(e, eJ(t)),
		...
		async function to(e) {
			const t = new Headers(tn(null == e ? void 0 : e.headers, await e6(e)));
			if (!t.has("content-type")) {
				const r = tt(null == e ? void 0 : e.body) ? "application/json" : null;
				r && t.set("content-type", r);
			}
			return t;
		}
		var ti = class e extends Error {
			constructor(t, r) {
		...
		async function ts(e, t) {
			...
			if (r.issues) throw new ti(r.issues);
			return r.value;
		}
		var tl = ["get", "post", "put", "patch", "delete"],
			tu = async (e, t) => {
				var r, a, n, o, i, s, l, u;
				let d,
					{ hooks: c, url: f, options: p } = await e9(e, t),
					h = (function (e) {
						if (null == e ? void 0 : e.customFetchImpl)
							return e.customFetchImpl;
						if ("u" > typeof globalThis && ta(globalThis.fetch))
							return globalThis.fetch;
						if ("u" > typeof window && ta(window.fetch)) return window.fetch;
				...
				const y = null != (s = null == t ? void 0 : t.jsonParser) ? s : tr,
					I = await A.text(),
					w = (function (e) {
						try {
				...
				if (null == t ? void 0 : t.retry) {
					const r = (function (e) {
							if ("number" == typeof e)
								return new e7({ type: "linear", attempts: e, delay: 1e3 });
							switch (e.type) {
								case "linear":
									return new e7(e);
								case "exponential":
					...
					if (await r.shouldAttemptRetry(a, A)) {
						...
						return (
							...
							await tu(e, e5(e4({}, t), { retryAttempt: a + 1 }))
						);
					}
				}
				if (null == t ? void 0 : t.throw)
					throw new e3(A.status, A.statusText, w ? N : I);
				return {
					data: null,
					error: e5(e4({}, N), { status: A.status, statusText: A.statusText }),
				};
			};
		const td = /[\p{Ll}\d]+|\p{Lu}+(?!\p{Ll})|\p{Lu}[\p{Ll}\d]+|\p{Lo}+/gu,
			tc = /['\u2019]/g;
		function tf(e) {
			return (e.replace(tc, "").match(td) ?? [])
				.map((e) => e.toLowerCase())
				.join("-");
		}
		var tp = e.i(92479);
			...
			[
				...
				function (e) {
					...
					const {
							...
							let r,
								...
								c =
									(function (e, t, r, a) {
										...
										if (window.location) return eS(window.location.origin, t);
								...
								T =
									((t = {
											...
											e
												? (function (e, t = { strict: !0 }) {
														return (function (e, t = {}) {
															...
															const i = e.trim(),
																s = i.toLowerCase();
															if (s.length <= 9 && s in eD) return eD[s];
															if (!eU.test(i)) {
																if (r)
																	throw SyntaxError(
										...
										plugins: [
											h,
											...(O.plugins || []),
											...(e?.disableDefaultFetchPlugins ? [] : [ex]),
											...f,
										],
									}),
									async function (e, r) {
										...
										const {
											...
											u = (e) => {
												if (s || e$().isOnline) {
													...
													if (e?.event === "poll") {
														(l.lastSessionRequest = ez()), t();
														return;
													}
													if (e?.event === "visibilitychange") {
														if (ez() - l.lastSessionRequest < 5) return;
														(l.lastSessionRequest = ez()), t();
														return;
													}
													t();
										...
										return {
											init: () => {
												l.isInitialized ||
													...
													(l.unsubscribeBroadcast = ek().subscribe(() => {
														u({ event: "storage" });
													})),
													i &&
														(l.unsubscribeFocus = eF().subscribe(() => {
															u({ event: "visibilitychange" });
														})),
													(l.unsubscribeOnline = e$().subscribe((e) => {
														e && u({ event: "visibilitychange" });
													})),
													(l.unsubscribeSignal = a.listen(() => {
														t();
													})),
													(l.cleanupBroadcastSetup = ek().setup()),
													(l.cleanupFocusSetup = eF().setup()),
													(l.cleanupOnlineSetup = e$().setup()));
											},
											cleanup: () => {
												l.isInitialized &&
													(l.pollInterval &&
							...
							for (const t of v)
								t.getActions && (y = eK(t.getActions?.(T, S, e) ?? {}, y));
							return {
								get baseURL() {
									return c;
								},
					...
					for (const [e, t] of Object.entries(n))
						u[
							...
							(function (e, t = {}) {
								const r = (0, tp.useRef)(e.get()),
									{ keys: a, deps: n = [e, a] } = t,
									o = (0, tp.useCallback)((t) => {
										const n = (e) => {
									...
									i = () => r.current;
								return (0, tp.useSyncExternalStore)(o, i, i);
							})(t);
					return (
	...
	(e) => {
		...
		const m = (0, h.createAuthClient)({
			...
			fetchOptions: {
				onError(e) {
					429 === e.error.status &&
						E.toast.error("Too many requests. Please try again later.");
				},
			},
		});
		e.s(["authClient", 0, m]);
