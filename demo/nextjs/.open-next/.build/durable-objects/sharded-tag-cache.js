...
// node_modules/.pnpm/@opennextjs+cloudflare@1.19_7bf66971aa7457561755ae2e85361605/node_modules/@opennextjs/cloudflare/dist/api/durable-objects/sharded-tag-cache.js
import { DurableObject } from "cloudflare:workers";

// node_modules/.pnpm/@opennextjs+cloudflare@1.19_7bf66971aa7457561755ae2e85361605/node_modules/@opennextjs/cloudflare/dist/api/cloudflare-context.js
var cloudflareContextSymbol = Symbol.for("__cloudflare-context__");

// node_modules/.pnpm/@opennextjs+cloudflare@1.19_7bf66971aa7457561755ae2e85361605/node_modules/@opennextjs/cloudflare/dist/api/overrides/internal.js
...
var DOShardedTagCache = class extends DurableObject {
	...
	async getTagData(tags) {
		...
		try {
			...
			debugCache(
				"DOShardedTagCache",
				`getTagData tags=${tags} -> ${result.length} results`,
			);
	...
	async getLastRevalidated(tags) {
		...
		const timeMs =
			values.length === 0
				? 0
				: Math.max(...values.map(({ revalidatedAt }) => revalidatedAt));
		debugCache(
			"DOShardedTagCache",
	...
	async hasBeenRevalidated(tags, lastModified) {
		...
		const revalidated = Object.values(data).some(
			({ revalidatedAt }) => revalidatedAt > lastModifiedOrNowMs,
		);
		debugCache(
			"DOShardedTagCache",
	...
	async getRevalidationTimes(tags) {
		const data = await this.getTagData(tags);
		return Object.fromEntries(
			Object.entries(data).map(([tag, { revalidatedAt }]) => [
				tag,
				revalidatedAt,
			]),
		);
	...
	async writeTags(tags, lastModified) {
		...
		debugCache(
			"DOShardedTagCache",
			`writeTags tags=${JSON.stringify(tags)} time=${nowMs}`,
		);
