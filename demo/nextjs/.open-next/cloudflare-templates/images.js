...
export async function handleImageRequest(requestURL, requestHeaders, env) {
	...
	if (parseResult.url.startsWith("/")) {
		...
		try {
			fetchImageResult = await fetchWithRedirects(
				parseResult.url,
				7_000,
...
function validateWidthQueryParameter(requestURL) {
	...
	const sizeValid =
		__IMAGES_DEVICE_SIZES__.includes(width) ||
		__IMAGES_IMAGE_SIZES__.includes(width);
	if (!sizeValid) {
		const result = {
			ok: false,
 ...
 */
const SUPPORTED_CDN_CGI_INPUT_TYPES = new Set([
	JPEG,
	PNG,
	GIF,
	WEBP,
