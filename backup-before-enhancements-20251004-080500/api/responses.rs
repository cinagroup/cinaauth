...
impl<T> ApiResponse<T> {
    ...
    pub fn unauthorized_typed(code: &str, message: impl Into<String>) -> ApiResponse<T> {
        ApiResponse::<()>::error(code, message).cast()
    }

    /// Create a conflict response for any type
    pub fn conflict_typed(code: &str, message: impl Into<String>) -> ApiResponse<T> {
        ApiResponse::<()>::conflict(code, message).cast()
    }
    ...
    fn into_response(self) -> Response {
        let status = if self.success {
            ...
            match self.error.as_ref().map(|e| e.code.as_str()) {
                Some("UNAUTHORIZED") | Some("INVALID_CREDENTIALS") | Some("AUTH_ERROR") | Some("INVALID_TOKEN") => StatusCode::UNAUTHORIZED,
                Some("FORBIDDEN") => StatusCode::FORBIDDEN,
                Some("NOT_FOUND") => StatusCode::NOT_FOUND,
                Some("VALIDATION_ERROR") => StatusCode::BAD_REQUEST,
                Some("USERNAME_EXISTS") | Some("EMAIL_EXISTS") => StatusCode::CONFLICT,
