...
fn bench_serialization(c: &mut Criterion) {
    ...
    let token = AuthToken {
        ...
        token_type: Some("Bearer".to_string()),
        expires_at: chrono::Utc::now() + chrono::Duration::hours(1),
        scopes: vec!["read".to_string(), "write".to_string(), "admin".to_string()].into(),
        issued_at: chrono::Utc::now(),
        auth_method: "oauth2".to_string(),
        subject: Some("subject123".to_string()),
        issuer: Some("https://auth.example.com".to_string()),
        client_id: Some("client123".to_string()),
        user_profile: None,
        permissions: vec!["read".to_string(), "write".to_string(), "admin".to_string()].into(),
        roles: vec!["user".to_string(), "admin".to_string()].into(),
