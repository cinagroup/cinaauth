...
impl AuthFramework {
    ...
    /// Get a reference to the framework's configuration.
    ...
    ///
    /// This is the primary method for user registration. It handles:
    ...
    ///
    /// # Arguments
    /// * `username` - Unique username (3-32 chars, alphanumeric + _ -)
    /// * `email` - Valid email address
    /// * `password` - Password meeting security requirements
    ///
    /// # Returns
    /// The new user's unique ID on success
    ///
    /// # Errors
    /// Returns error if:
    ...
    pub async fn register_user(
        ...
        if let Err(e) = crate::utils::validation::validate_email(email) {
            return Err(AuthError::validation(format!("Invalid email: {}", e)));
        }

        // Validate password against security policy
        let password_policy = crate::utils::validation::PasswordPolicy {
            min_length: self.config.security.min_password_length,
            max_length: 128,
            require_lowercase: self.config.security.require_lowercase,
            require_uppercase: self.config.security.require_uppercase,
            require_digit: self.config.security.require_digit,
            require_special: self.config.security.require_special,
            banned_passwords: std::collections::HashSet::new(),
            min_entropy: 3.0,
        };

        if let Err(e) =
            crate::utils::validation::validate_password_enhanced(password, &password_policy)
        {
        ...
        self.storage
            ...
            .await
            .map_err(|e| {
                error!("Failed to store user credentials: {:?}", e);
                AuthError::internal(format!("Failed to create user account: {}", e))
            })?;
        ...
        if let Err(e) = self
            ...
            .await
        {
            error!("Failed to store email mapping: {:?}", e);
            // Rollback: delete the user credentials
        ...
        // Store user ID mapping for lookups by ID
        let user_id_key = format!("user:id:{}", user_id);
        if let Err(e) = self
            .storage
            .store_kv(&user_id_key, username.as_bytes(), None)
            .await
        {
            error!("Failed to store user ID mapping: {:?}", e);
            // Rollback: delete both previous entries
            let _ = self.storage.delete_kv(&username_key).await;
        ...
        let audit_event = crate::audit::AuditEvent {
            id: String::new(),
            event_type: crate::audit::AuditEventType::UserCreated,
            timestamp: std::time::SystemTime::now(),
            user_id: Some(user_id.clone()),
            session_id: None,
            outcome: crate::audit::EventOutcome::Success,
            risk_level: crate::audit::RiskLevel::Low,
            description: format!("User {} registered successfully", username),
            details: {
            ...
            resource: Some(crate::audit::ResourceInfo {
            ...
            actor: crate::audit::ActorInfo {
                ...
                actor_id: "registration".to_string(),
                actor_name: None,
                roles: vec![],
            },
            correlation_id: None,
        };
        let _ = self.audit_manager.log_event(audit_event).await;
    ...
    ///
    /// # Returns
    /// User data as JSON value if found
    pub async fn get_user_by_username(&self, username: &str) -> Result<serde_json::Value> {
        let username_key = format!("user:credentials:{}", username);
        match self.storage.get_kv(&username_key).await? {
            Some(data) => {
                let user_data: serde_json::Value = serde_json::from_slice(&data).map_err(|e| {
                    AuthError::internal(format!("Failed to parse user data: {}", e))
                })?;
                Ok(user_data)
            }
            None => Err(AuthError::authorization("User not found")),
        }
    }
    ...
    pub async fn get_user_by_id(&self, user_id: &str) -> Result<serde_json::Value> {
        // First get the username from user ID mapping
        let user_id_key = format!("user:id:{}", user_id);
        match self.storage.get_kv(&user_id_key).await? {
            Some(username_bytes) => {
                let username = String::from_utf8(username_bytes)
                    .map_err(|e| AuthError::internal(format!("Failed to parse username: {}", e)))?;
    ...
    pub async fn update_user_password(&self, username: &str, new_password: &str) -> Result<()> {
        ...
        if let Err(e) =
            crate::utils::validation::validate_password_enhanced(new_password, &password_policy)
        {
            return Err(AuthError::validation(format!(
                "Password validation failed: {}",
                e
        ...
        let mut user_data = self.get_user_by_username(username).await?;

        // Hash new password
        let password_hash = crate::utils::password::hash_password(new_password)?;
        ...
        self.storage
            ...
            .map_err(|e| {
                ...
                AuthError::internal(format!("Failed to update password: {}", e))
            })?;

        info!("Password updated successfully for user: {}", username);

        // Log audit event
        if let Some(user_id_str) = user_data["user_id"].as_str() {
            let audit_event = crate::audit::AuditEvent {
                id: String::new(),
                event_type: crate::audit::AuditEventType::UserPasswordChanged,
                timestamp: std::time::SystemTime::now(),
                user_id: Some(user_id_str.to_string()),
                session_id: None,
                outcome: crate::audit::EventOutcome::Success,
                risk_level: crate::audit::RiskLevel::Medium,
                description: format!("Password changed for user {}", username),
                details: std::collections::HashMap::new(),
                request_metadata: crate::audit::RequestMetadata::default(),
                resource: Some(crate::audit::ResourceInfo {
                    resource_type: "user".to_string(),
                    resource_id: user_id_str.to_string(),
                    resource_name: Some(username.to_string()),
                    attributes: std::collections::HashMap::new(),
                }),
                actor: crate::audit::ActorInfo {
                    actor_type: "user".to_string(),
                    actor_id: user_id_str.to_string(),
    ...
    pub async fn delete_user(&self, username: &str) -> Result<()> {
        ...
        // Get user data first to retrieve user_id and email
        let user_data = self.get_user_by_username(username).await?;

        let user_id = user_data["user_id"]
            .as_str()
            .ok_or_else(|| AuthError::internal("User data missing user_id"))?;
        let email = user_data["email"]
            .as_str()
            .ok_or_else(|| AuthError::internal("User data missing email"))?;
        ...
        // Best effort deletion (don't fail if some keys don't exist)
        let _ = self.storage.delete_kv(&username_key).await;
        let _ = self.storage.delete_kv(&email_key).await;
        let _ = self.storage.delete_kv(&user_id_key).await;
        ...
        let audit_event = crate::audit::AuditEvent {
            id: String::new(),
            event_type: crate::audit::AuditEventType::UserDeleted,
            timestamp: std::time::SystemTime::now(),
            user_id: Some(user_id.to_string()),
            session_id: None,
            outcome: crate::audit::EventOutcome::Success,
            risk_level: crate::audit::RiskLevel::High,
            description: format!("User {} deleted", username),
            details: std::collections::HashMap::new(),
            request_metadata: crate::audit::RequestMetadata::default(),
            resource: Some(crate::audit::ResourceInfo {
                resource_type: "user".to_string(),
