...
"#,
    ...
    fn generate_types(&self) -> Result<String, Box<dyn std::error::Error>> {
        let types_code = r#"
/**
 * TypeScript type definitions for Cinaauth RBAC
 */
...
"#,
    ...
    fn generate_utils(&self) -> Result<String, Box<dyn std::error::Error>> {
        let utils_code = r#"
/**
 * Utility functions for Cinaauth SDK
 */
...
"#;
    ...
    fn generate_index(&self) -> Result<String, Box<dyn std::error::Error>> {
        let index_code = format!(
            r#"
/**
 * Cinaauth Enhanced SDK with RBAC Support
 *
 * @version {version}
...
// Default export
export default CinaauthSdk;

// Create convenience function
export function createCinaauthClient(config: ClientConfig): CinaauthSdk {{
  return new CinaauthSdk(config);
}}
"#,
  ...
  "version": "1.0.0",
  "description": "Enhanced cinaauth client library with RBAC support",
  "main": "dist/index.js",
...
"#
    ...
    fn generate_readme(&self) -> Result<String, Box<dyn std::error::Error>> {
        let readme = r#"# cinaauth JavaScript/TypeScript SDK

Enhanced client library for Cinaauth with comprehensive RBAC support powered by role-system v1.0.
...
```typescript
import {{ CinaauthSdk }} from '@authframework/client';

// Initialize the client
const auth = new CinaauthSdk({{
